...
cat(file=fileLog,append=F,paste(sep="","This is PCVPA_splineTobitRegressionUnknownKnotLocations_20220110_forPub.R\n\n\tseed = < ",seed," >\n\tdate = < ",Sys.Date()," >\n\tinFile = < ",inFile," >\n\n"))
...
parsMat<-matrix(nrow=length(st),ncol=1+3+1+4)
colnames(parsMat)<-c("k","Knot1","Knot2","Knot3","Intercept","Slope1","Slope2","Slope3","Slope4")
...
write.csv(parsMat,row.names=T,quote=F,file=paste(sep="","fit_strains",paste(collapse="-",st),"_parameters.csv"))
...
pdf(width=24,height=16,file="plotAllSerotypes_PCVPA_noGmcCI.pdf")
grid.arrange(grobs=plotList,nrow=3,padding = unit(0.1, "line"),bottom=textGrob("age (months)", gp = gpar(fontsize = 24)),left=textGrob("IgG concentration (ug/mL)", rot=90, gp = gpar(fontsize = 24)))
dev.off()

s<-"23F"
pdf(width=6,height=5,file="plotSerotype23F_PCVPA.pdf")
print(ggPlotFun(fit=fitList[[s]],strain=s,varNameImpDL=paste(sep="","res",s,"_num_ImpDL"),DL=0.15,colModel="steelblue2",colModelCI="steelblue",colChangepoints="orange",plotModel=TRUE,cexAnnot=12,doLegend = TRUE,doGeoMConfInt = TRUE)$g)
dev.off()
