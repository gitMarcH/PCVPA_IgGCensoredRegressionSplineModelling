...
residuals.censReg<-function(fit,s,scale="log"){
  logy<-fit$coefs[1]+fit$coefs[2]*fit$dat$ageMonths
  if(fit$k>0){
    for(i in 1:fit$k){
      logy<-logy+fit$coefs[2+i]*ifelse((fit$dat$ageMonths-fit$coefs[2+fit$k+1+i])>=0,fit$dat$ageMonths-fit$coefs[2+fit$k+1+i],0)
    }
  }
  
  if(scale=="log"){
    res<-log(fit$dat[,paste(sep="","res",s,"_num")])-logy
  }else{
    y<-exp(logy)
    res<-fit$dat[,paste(sep="","res",s,"_num")]-y
  }
...
residuals.censReg<-function(fit,s,scale="log"){
  # very basic residual function
  # scale="log" is the default given that these models are fitted to log-transformed data
  ...
  if(scale=="log"){
    out$data<-log(fit$dat[,paste(sep="","res",s,"_num")])
    out$predictions<-logy
    out$residuals<-log(fit$dat[,paste(sep="","res",s,"_num")])-logy
  }else if(scale=="response"){
    out$data<-fit$dat[,paste(sep="","res",s,"_num")]
    out$predictions<-exp(logy)
    out$residuals<-fit$dat[,paste(sep="","res",s,"_num")]-out$data
  }else{
    stop("Argument 'scale' needs to be one of 'link' or 'response'")
  }
...
grid.arrange(grobs=plotList,nrow=3,padding = unit(1, "line"),bottom=grid::textGrob("age (months)", gp = grid::gpar(fontsize = 22)),left=grid::textGrob("IgG concentration (ug/mL)", rot=90, gp = grid::gpar(fontsize = 22)))
dev.off()

s<-"23F"
pdf(width=8,height=4.5,file="plotSerotype23F_PCVPA.pdf")
...
# residuals for model diagnostics can be easily computed; by default these are done on the log scale as the models were fitted to log-transformed data
res.9V<-residuals.censReg(fitList[["9V"]],s="9V")
res.23F<-residuals.censReg(fitList[["23F"]],s="23F")
